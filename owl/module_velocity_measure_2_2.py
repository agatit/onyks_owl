...
class Module(module_base.Module):
    ...
    def task_process(self, input_task_data, input_stream ):
        ...
        max_q_lenght = self.params.get("max_q_lenght", 100)
                ...
                ####################################################################                  OBLICZANIE PREDKOSCI                         ###########################################################################    
                    ...
                    for i,(new,old) in enumerate(zip(good_new, good_old)):
                        ...
                        if abs(int(b) - int(d)) < y_max_velocity and abs(int(a) - int(c)) >x_min_velocity and abs(int(a) - int(c)) < x_max_velocity: 
                            q_2 = np.append(q_2, (a - c))
                            
                            if len(q_2) >= max_q_lenght:
                                q_2 = np.delete(q_2, 0)
                    ...
                    if q_2 is not None:
                        if len(q_2) > min_v_tab_length:
                            p_min = np.percentile(q_2, 25)
                            p_max = np.percentile(q_2, 75)
                            v_f_min  = q_2[q_2 > p_min ]
                            v_f_max  = q_2[q_2 < p_max ]

                            v_f2 = np.intersect1d(v_f_min,v_f_max)
                            v_avg = np.average(v_f2)
                            print(v_avg)
