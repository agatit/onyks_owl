...
class CarSpeedEstimator:
    def __init__(
        ...
        self.lk_params = dict( winSize  = (x_max_velocity+3, y_max_velocity+3), # TODO: 3 to minimum, do przemyślenia!!!
                        maxLevel = 2,
    ...
    def next(self, frame, pts=1/30, debug=None):
        ...
        if len(velocities) > 0:
            ...
            self.velocity_history = np.append(self.velocity_history, velocities[(velocities[:,0] >= perc_low) & (velocities[:,0] <= perc_high),:], axis=0)
        ...
        if len(self.velocity_history) > 0:
            ...
            to_avarage = self.velocity_history[(self.velocity_history[:,0] >= perc_low) & (self.velocity_history[:,0] <= perc_high)]
