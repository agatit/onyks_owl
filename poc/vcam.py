'''
Based on https://github.com/kaustubh-sadekar/VirtualCam.git
MIT license 2022
'''
...
class vcam:
	...
	def __init__(self,H=400,W=400):
		...
		self.H = H
		self.W = W
		self.ox = W//2 # optical center in camera matrix
		self.oy = H//2 # optical center in camera matrix
		self.alpha = math.radians(0) # roataions
		self.beta =  math.radians(0)
		self.gamma = math.radians(0)
		self.Tx = 0 # translation
		self.Ty = 0
		self.Tz = 0
		self.K = 0 # camera matrix
		self.R = 0 # all rotations matrix
		self.sh = 0 # shear factor
		self.P = 0 # never used ????
		# self.RT = 0 # composite rotation/translation matrix
		self.KpCoeff = np.array([0.0]*8) # k1,k2,p1,p2,k3,k4,k5,k6
		self.focus = 4 # Focal length of camera in mm
